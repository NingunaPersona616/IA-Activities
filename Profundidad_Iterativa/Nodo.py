...
class Node:
    ...
    def expandNodes(self, openNodes, level, path, maxDepth):
        if level == maxDepth:
            return None
        ...
        if self.up == True:
            ...
            if(self.isOpened(newData, openNodes)) == False:    #Si el nodo por abir no ha sido abierto
                ...
                result = self.Nodes[0].expandNodes(openNodes, level+1, path, maxDepth) #Expandimos el nuevo nodo creado y busca la sol en sus hijos
