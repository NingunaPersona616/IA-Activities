...
def DFS_Interface(estado, estadosExplorados, maxDepth):
    root = Nodo(estado, None)
    solution = DFS(root, estadosExplorados, 0, maxDepth)
    return solution
    

def DFS(nodoActual, estadosExplorados, depth, maxDepth):
    ...
    if not nodoActual.moves:  #Si el nodo actual no tiene movs regresa nulo
        ...
        for move in nodoActual.moves:   #Movimiento por movimiento creamos y expandimos cada nuevo nodo hijo
            ...
            Solution = DFS(nodoSig, estadosExplorados, depth+1, maxDepth) #Expandimos el nuevo nodo hijo recien creado
...
if __name__ == '__main__': 
    ...
    while solution is None and actualDepth < maxDepth:
        estados_explorados = []    
        solution = DFS_Interface(laberinto.inicio, estados_explorados, actualDepth)
        actualDepth += 1

    print("Solucion encontrada en el nivel:", actualDepth)
